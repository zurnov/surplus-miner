...
const SSH_OPTS =
  process.env.SSH_OPTS || '-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null';
...
const REQUEST_LOG_IGNORE_PREFIX = (process.env.REQUEST_LOG_IGNORE_PREFIX || '/api/miners')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean);
...
mqttClient.on('connect', () => {
  ...
  mqttClient.subscribe(topics, (err) => {
    if (!err) {
      ...
      // Don't end client, let it auto-reconnect
    }
  });
...
mqttClient.on('message', (topic, message) => {
  try {
    ...
    logger.warn('Could not parse MQTT message', {
      topic,
...
setInterval(() => {
  ...
  const totalConsumption =
    latestData.consumption.l1 + latestData.consumption.l2 + latestData.consumption.l3;
  const dataToSend = {
...
app.get('/api/overview', async (req, res) => {
  ...
  try {
    ...
    const combined = await papiRequest('summary+devs+pools', minerHost);
...
function applyMinerPower(host, logicalTarget) {
  return new Promise((resolve) => {
    ...
    parts.push(
      `sed -i -E 's/^([[:space:]]*power_target[[:space:]]*=[[:space:]]*)[0-9]+/\\1 ${logicalTarget}/' /etc/bosminer.toml`
    );
...
async function autoSetMinerPower() {
  ...
  try {
    ...
    for (const host of MINER_HOSTS)
      if (!Number.isFinite(lastAutoTargets[host])) lastAutoTargets[host] = MIN_POWER;
    ...
    try {
      measuredResults = await Promise.all(
        MINER_HOSTS.map((h) =>
          getMinerPower(h).catch((err) => ({ total: null, error: err && err.message }))
        )
      );
    } catch (e) {
    ...
    const measuredMap = Object.fromEntries(
      MINER_HOSTS.map((h, i) => [
        h,
        Number.isFinite(measuredResults[i]?.total) ? measuredResults[i].total : null,
      ])
    );
    const measuredSum = MINER_HOSTS.reduce(
      (s, h) => s + (Number.isFinite(measuredMap[h]) ? measuredMap[h] : 0),
      0
    );

    logger.debug('Measured miner powers', {
      measuredMap,
      measuredSum,
    ...
    if (minersAreShutDown) {
      if (resumeEnabled) {
        ...
        logger.debug(
          `Resume evaluation: gridPower=${gridPower.toFixed(1)}W, avgGridPower=${avgGridPower.toFixed(1)}W, measuredSum=${measuredSum.toFixed(1)}W`,
          {
        ...
        if (resumeCond) {
          if (!surplusHoldStartTs) {
            ...
            logger.info(
              `Resume arming: sustained surplus detected (${surplusAmount.toFixed(1)}W export). Waiting ${AUTO_RESUME_MINUTES} min before resume...`
            );
          }
          const heldMs = now - (surplusHoldStartTs || now);
          if (heldMs >= AUTO_RESUME_MINUTES * 60000) {
            logger.info(
              `Auto-resume triggered: surplus held for ${heldMs / 60000} minutes. Starting all miners.`
            );
            startAllMiners().then(() => {
        ...
        } else if (surplusHoldStartTs) surplusHoldStartTs = null;
      } else {
    ...
    if (shutdownEnabled) {
      ...
      if (shutdownCond) {
        if (!noSurplusStartTs) {
          ...
          logger.info(
            `Shutdown arming: no-surplus/deficit detected. Waiting ${AUTO_SHUTDOWN_MINUTES} min...`
          );
        }
    ...
    if (DISCRETE_MODE) {
      ...
      logger.debug('Discrete mode decision', {
        desiredTotal,
        measuredSum,
        lastAutoTargets,
        miners: MINER_HOSTS.length,
      });
    ...
    if (avgGridPower < GRID_SURPLUS_THRESHOLD - HYSTERESIS) {
      ...
      logger.debug(
        `Surplus distribution: desiredTotal=${desiredTotal}W, measuredSum=${currentSum}W, remaining=${remaining}W, capacity=${maxCapacity}W`
      );
      ...
      const hostsByPower = hostsFair
        .slice()
        .sort((a, b) => (lastAutoTargets[b] || MIN_POWER) - (lastAutoTargets[a] || MIN_POWER));

      let reducibleTotal = 0;
      for (const h of hostsByPower)
        reducibleTotal += Math.max(0, (lastAutoTargets[h] || MIN_POWER) - MIN_POWER);
      if (needReduce > reducibleTotal) needReduce = reducibleTotal;

      logger.debug(
        `Reducing miners: avgGrid=${avgGridPower.toFixed(1)}W, needReduce=${needReduce}W, reducibleTotal=${reducibleTotal}W`
      );
    ...
    if (actions.length) {
      Promise.allSettled(actions).then(() => {
        ...
        logger.debug('Applied auto-control changes', {
          perMinerTargets: Object.fromEntries(MINER_HOSTS.map((h) => [h, lastAutoTargets[h]])),
        });
      });
    } else {
...
app.post('/api/auto-control', (req, res) => {
  if (typeof req.body.enable === 'boolean') {
    ...
    try {
      ...
      const targetsStr =
        MINER_HOSTS.map((h) => `${h}:${lastAutoTargets[h] || MIN_POWER}W`).join(', ') || 'none';
      logger.info(`Auto-control ${autoControlEnabled ? 'ENABLED' : 'DISABLED'}`, {
        config: cfg,
    ...
    io.emit('auto_control_update', {
      ...
      perMinerTargets: Object.fromEntries(
        MINER_HOSTS.map((h) => [h, lastAutoTargets[h] || MIN_POWER])
      ),
    });
...
app.get('/api/miners', async (req, res) => {
  ...
  async function fetchAll(host) {
    ...
    let combined;
    try {
      combined = await papiRequest('summary+devs+pools', host);
    } catch {
      combined = null;
    }
      ...
      MINER_HOSTS.map(async (host, idx) => {
        try {
          ...
          const activePool = (pools || []).find(
            (pool) => pool.Status === 'Alive' || /alive/i.test(pool.Status || '')
          );
          ...
          const maxTemp =
            devs && devs.length
              ? Math.max(...devs.map((dev) => Number(dev.Temperature || 0)))
              : null;
...
async function detectInitialMinerStates() {
  ...
  try {
    ...
    for (const r of results) {
      ...
      logger.info(
        `${r.host} -> ${r.state}${Number.isFinite(r.power) ? ` (power_target ~${r.power}W)` : ''}`
      );
    }
...
app.use('/api/auto-control', (req, res, next) => {
  if (
    req.method === 'POST' &&
    req.body &&
    typeof req.body.enable === 'boolean' &&
    req.body.enable
  ) {
